...
1. X86有几个特权级？
   - 四个特权级，从level 0到level 3。一般操作系统只使用0和3。


2. 不同特权级有什么区别？
   - 不同特权级下可以运行的指令不同：如特权指令只能在ring 0下运行
   - 不同特权级下同一指令能够访问的数据范围不一样
   - 不同特权级下同一指令的行为可能不同


3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
   - RPL存储在数据段寄存器段选择子末两位，表示当前要访问的数据段对应的特权级（进程对段访问时的请求权限）
   - CPL存储在代码段CS寄存器段选择子末两位，表示当前代码段对应的特权级（当前进程的权限级别）
   - DPL存储在段描述符/门描述符中，表示访问目标的特权级
   - 访问门时：CPL <= DPL（门）且 CPL >= DPL（段）
     - 门特权级要低，执行代码段特权级高，如此通过门
     - 通过门之后去访问特权级更高的段
...
1. 一条指令在执行时会有哪些可能的特权级判断？
   - 访问数据时，检查当前进程特权级CPL、请求特权级RPL与访问数据段DPL之间的关系。
   - 转移控制权时，应由低特权级代码转移到高特权级代码。
   - 发生中断或异常时，检查当前门描述符的特权级DPL和目标代码的特权级CPL。
2. 在什么情况下会出现特权级切换？
   - 当低特权级代码调用高特权级代码，或发生中断、异常，或从中断服务例程返回低特权级时。
3. int指令在ring0和ring3的执行行为有什么不同？
   - 压栈的内容不同：ring0下执行int不需要压入SS和ESP。
   - ring3下执行int软中断需要切换栈。


4. 如何利用int和iret指令完成不同特权级的切换？
   - 软件修改栈的内容，通过int进行软中断，通过iret前往想要切换的特权级。


5. TSS和Task Register的作用是什么？
   - TSS（Task-State Segment）任务状态段：保存任务信息，被OS用来进行任务管理。
     - 保存不同特权级下的寄存器信息。
     - IO端口权限
     - inner-level 栈指针
     - 上一个TSS链接
...
1. 一条指令执行时最多会出现多少次地址转换？
   - 最多两次：从虚拟地址（逻辑地址）到线性地址，再到物理地址。
2. 描述X86-32的MMU地址转换过程；
   - 从逻辑地址拆分得到段选择子（段号），查找GDT得到段描述符，从而得到段基址，与offset相加后得到线性地址空间中的地址。
   - 如果未启动页机制，则线性地址就是物理地址。如果启动页机制，则对线性地址拆分得到页目录（Page Directory）索引，与页目录基址（存储在寄存器CR3）相加得到获得页目录项，得到页表基址，与表内偏移相加后得到页表项，得到页基址，与offset相加得到实际的物理地址。
